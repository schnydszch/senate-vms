<html>
	...
	?>	
		...
		echo "<p style='color:white;font-size:x-large;'>Please find below similar employees (Click the name of employee to submit)"."</p>";
	...
	//echo "$idscanned";
		...
		$ret=mysqli_query($link,"SELECT cardnumber,firstname,surname FROM `borrowers` WHERE surname LIKE '%$idscanned%'||firstname LIKE '%$idscanned%'||cardnumber LIKE '%$idscanned%'");
		...
		while ($row=mysqli_fetch_array($ret)) {
			...
			<input type="hidden" id="hiddenField" name="id2" value="<?php echo $row['cardnumber']; ?>" />
			<a href="#" onClick="mySubmit();">
			<?php echo $row['firstname']." ".$row['surname'] ?></a><br/>			<p>
